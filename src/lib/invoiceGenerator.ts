...
export function generateWeeklyInvoice(data: InvoiceData) {
  ...
  doc.text(`Trading Days: ${data.tradingDays}`, 20, 115);
  ...
  doc.text('AMOUNT (₹)', 150, tableTop + 7);
  ...
  if (data.weeklyPnl > 0) {
    doc.text(
      `Commission Calculation: ${formatCurrency(data.weeklyPnl)} × ${data.commissionPercentage}% = ${formatCurrency(data.commissionAmount)}`,
      20,
      yPos
    );
  } else {
  ...
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(9);
  doc.text('Please make payment within 7 days of invoice date.', 20, yPos);
  yPos += 6;
  doc.text('For payment queries, please contact us.', 20, yPos);
  ...
  doc.text('Generated by Trade Tracker', 105, footerY + 12, { align: 'center' });
